...
- Laravel Octane + FrankenPHP is the selected HTTP runtime. Docker Compose is the canonical reproducible local integration environment. Laravel Reverb remains a separate realtime runtime, and queues perform asynchronous work when their approved Phase 3 stages are implemented.
...
- Do not install or configure runtime infrastructure outside its approved Phase 3 stage. Do not add Node.js as a primary backend, Next.js as the primary app, or microservices without evidence and an approved ADR.
